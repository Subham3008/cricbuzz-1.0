...
import { LayoutDashboard, Users, ShieldAlert, MonitorPlay, Activity, Settings, Plus, HelpCircle, LogOut, X, Calendar } from 'lucide-react';
...
const Sidebar = ({ onClose }) => {
  ...
  const navItems = [
    ...
    { name: 'Schedule', path: '/home/schedule', icon: Calendar },
  ...
  return (
    <aside className="w-[240px] bg-[#516170] flex flex-col h-full font-sans">
      {/* Header */}
      <div className="pt-6 px-6 pb-8 flex items-start justify-between">
        <div>
        ...
        {/* Mobile close button */}
        <button onClick={onClose} className="lg:hidden p-1.5 text-[#a5b4c3] hover:text-white transition-colors">
          <X size={20} />
        </button>
      </div>
        ...
        <nav className="space-y-1 px-4">
          {navItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center px-3 py-3 rounded-md transition-colors group text-[#d1d9e0] hover:bg-[#3e4d5c] hover:text-white`}
            >
      ...
      <div className="px-5 pb-4 mt-auto">
        <div className="space-y-1">
          <button
            onClick={() => { navigate('/home/schedule'); if (onClose) onClose(); }}
          ...
          <button onClick={() => { navigate('/home/support'); if (onClose) onClose(); }} className="w-full flex items-center px-3 py-2 text-[#d1d9e0] hover:text-white transition-colors">
